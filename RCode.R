...
for(i in 1:length(LIST)){
  ...
  writeRaster(NDVI,paste0("********************",YEAR[i],".tif"),overwrite=T)
}
...
for(i in 1:length(LIST)){
  ...
  writeRaster(NDVI,paste0("********************",YEAR[i],".tif"))
}
...
for(i in 1:length(LIST)){
  ...
  NDVI = rast(LIST[i])
  NDVI = project(NDVI,GRID)
  writeRaster(NDVI,paste0("********************_",YEAR[i],".tif"))
}
...
# Load 'Image Count'
LIST.IC = list.files("********************",full.names=T)
NUMB = sds(LIST.IC)
...
NDVIL =  list.files("********************",recursive = T,full.names=T,pattern="NDVI")
YEAR = 1984:2021
...
SSL =  list.files("********************",recursive = T,full.names=T,pattern="Prob")
YEAR = 1984:2021
...
PERML =  list.files("********************",recursive = T,full.names=T,pattern="Perm")
YEAR = 1984:2021
...
setwd("********************") # <- set your wd
LIST = list.files("********************",full.names=T,pattern="NDVI")
NAMES = list.files("",pattern="NDVI")
...
# LIST IMAGES
LIST.NDVI = list.files("********************",full.names=T,pattern="NDVI")
NAMES = list.files("********************",pattern="NDVI")
NAMES = StrRight(NAMES,29)
...
polygon(y=c(DF.NUMBTOT$V2-DF.NUMBTOTsd$V2,rev(DF.NUMBTOT$V2+DF.NUMBTOTsd$V2)),
        x=c(DF.NUMBTOT$YEAR,rev(DF.NUMBTOT$YEAR)),col=adjustcolor("royalblue",alpha.f = 0.2),border = NA)
...
arrows(x0 = bp[2],x1 = bp[2],y0=round(mean(PERC.POS0.01),2), y1=c(round(mean(PERC.POS0.01),2)+sd(PERC.POS0.01)),angle=90,lwd=2)
arrows(x0 = bp[2],x1 = bp[2],y0=round(mean(PERC.POS0.01),2), y1=c(round(mean(PERC.POS0.01),2)-sd(PERC.POS0.01)),angle=90,lwd=2)
